...
CSV_FILE = 'rustic_timber_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_rustic_timber_smokehouse.html')

@app.route('/api/order_rustic_timber', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_rustic_timber', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_rustic_timber', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
