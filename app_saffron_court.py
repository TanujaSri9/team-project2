...
CSV_FILE = 'saffron_court_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_saffron_court.html')

@app.route('/api/order_saffron_court', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_saffron_court', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_saffron_court', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
