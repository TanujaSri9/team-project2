...
CSV_FILE = 'lotus_mist_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_lotus_mist.html')

@app.route('/api/order_lotus_mist', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_lotus_mist', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_lotus_mist', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
