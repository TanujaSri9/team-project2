...
CSV_FILE = 'gaucho_pride_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_gaucho_pride_churrascaria.html')

@app.route('/api/order_gaucho_pride', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_gaucho_pride', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_gaucho_pride', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
