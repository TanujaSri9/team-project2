...
CSV_FILE = 'curry_leaf_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_curry_leaf.html')

@app.route('/api/order_curry_leaf', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_curry_leaf', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_curry_leaf', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
