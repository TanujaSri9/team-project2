...
CSV_FILE = 'kimchi_soul_bbq_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_kimchi_soul.html')

@app.route('/api/order_kimchi_soul', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_kimchi_soul', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_kimchi_soul', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
