...
CSV_FILE = 'iberia_social_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_iberia_social_tapas.html')

@app.route('/api/order_iberia_social', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_iberia_social', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_iberia_social', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
