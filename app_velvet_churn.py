...
CSV_FILE = 'velvet_churn_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_velvet_churn.html')

@app.route('/api/order_velvet_churn', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_velvet_churn', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_velvet_churn', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
